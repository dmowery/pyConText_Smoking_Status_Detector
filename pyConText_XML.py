...
class mentionAnnotation(object):
    def __init__(self,tagObject,textSource=None,mentionClass=None,mentionid=None,annotatorid='SmokingStatus_FINDER',span=None,start=None,end=None,
    ...
    def setXML(self):
        ...
        start=self.getSpan()[0]; end = self.getSpan()[1]
...
def writeKnowtator(annotations,text_source): #test_source should be read automatically from the XML string
    ...
    docOut="";outString=""
    outpath=os.getcwd()+"/pyConTextNLP-0.5.1.3/src/eHOST_FILES/saved/"
    if not os.path.exists(outpath):
        os.mkdir(outpath)
        

    fout=open(outpath+"/%s.knowtator.xml"%text_source,"w")
    outString="""<?xml version="1.0" encoding="UTF-8"?>\n"""
    outString+="""<annotations textSource="%s">\n"""%text_source
    docOut=outString;annotXML=""
    ct=1
    for annotation in annotations:
        ...
        start=str(annotation.getStart()); end = str(annotation.getEnd())
        ...
        outStringClsMent+="""        <mentionClass id="%s">%s</mentionClass>\n"""%(annotation.getMentionClass(),annotation.getText().strip()) 
...
def main():
    ...
    lines=open(os.getcwd()+"/pyConTextNLP-0.5.1.3/src/reportsOutput/eHOST_annotations.txt","r").readlines()

    for l in lines[1:]:
        
        fname, term, span, clas = l.strip().split("\t")
